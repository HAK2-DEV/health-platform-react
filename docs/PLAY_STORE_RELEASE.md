...
- 데이터 삭제 요청 경로: 예 — ✅ **인앱 계정 삭제 있음**(프로필 > 계정 설정 > 회원 탈퇴, `delete_my_account` RPC, 전 데이터 즉시 파기).
  - ⚠️ Play는 인앱 외 **웹 삭제요청 URL**도 권장(미설치 사용자용). 지원 이메일 안내 페이지나 `/account-deletion` 간단 페이지 추가 검토.
