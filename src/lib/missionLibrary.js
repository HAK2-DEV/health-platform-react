// 추천 미션 라이브러리 — 운영자가 ProgramDetailPage 의 "➕ 미션 추가" 클릭 시 첫 화면
//
// 사용 흐름:
//   1. 운영자가 묶음 카드 선택 (예: "수면 회복 루틴")
//   2. 묶음 안의 미션 2~3개가 디폴트로 체크된 상태로 표시
//   3. 운영자가 토글로 일부 선택/해제 + 점수/한도 미세 조정 가능
//   4. "선택된 N개 추가" → missions 일괄 INSERT
//
// 데이터 구조:
//   bundles: 묶음 카드 메타
//   missions: 각 묶음의 미션 디폴트값 (MissionCreateModal 의 INSERT 페이로드와 동일 스키마)
//
// 본인의 카테고리 균형:
//   사진 1 레퍼런스 5개 (식단/수면/수분/마음챙김/금연) + WALKING + EMPATHY 추가 = 7개

export const MISSION_LIBRARY = [
  {
    key: 'diet_balance',
    title: '식단 밸런스 챌린지',
    description: '균형 잡힌 식습관 만들기',
    category: 'DIET',
    emoji: '🥗',
    missions: [
      {
        title: '아침 식단 사진',
        instruction: '오늘 아침에 먹은 음식을 사진으로 인증해요',
        icon: 'diet.png',
        requires_image: true,
        requires_numeric: false,
        requires_note: false,
        verification_type: 'AUTO',
        point: 5,
        daily_limit: 1,
      },
      {
        title: '점심 식단 사진',
        instruction: '오늘 점심에 먹은 음식을 사진으로 인증해요',
        icon: 'diet.png',
        requires_image: true,
        requires_numeric: false,
        requires_note: false,
        verification_type: 'AUTO',
        point: 5,
        daily_limit: 1,
      },
      {
        title: '저녁 식단 사진',
        instruction: '오늘 저녁에 먹은 음식을 사진으로 인증해요',
        icon: 'diet.png',
        requires_image: true,
        requires_numeric: false,
        requires_note: false,
        verification_type: 'AUTO',
        point: 5,
        daily_limit: 1,
      },
    ],
  },

  {
    key: 'sleep_recovery',
    title: '수면 회복 루틴',
    description: '숙면으로 컨디션 회복하기',
    category: 'SLEEP',
    emoji: '🌙',
    missions: [
      {
        title: '취침 시간 기록',
        instruction: '오늘 잠든 시간을 시·분 단위 숫자로 기록해요 (예: 23.5 = 23시 30분)',
        icon: 'sleep.png',
        requires_image: false,
        requires_numeric: true,
        requires_note: false,
        verification_type: 'AUTO',
        point: 5,
        daily_limit: 1,
      },
      {
        title: '아침 기상 기록',
        instruction: '오늘 일어난 시간을 시·분 단위 숫자로 기록해요',
        icon: 'sleep.png',
        requires_image: false,
        requires_numeric: true,
        requires_note: false,
        verification_type: 'AUTO',
        point: 5,
        daily_limit: 1,
      },
      {
        title: '수면 한 줄 소감',
        instruction: '오늘 수면 어땠나요? 한 줄로 남겨주세요',
        icon: 'sleep.png',
        requires_image: false,
        requires_numeric: false,
        requires_note: true,
        verification_type: 'AUTO',
        point: 5,
        daily_limit: 1,
      },
    ],
  },

  {
    key: 'hydration',
    title: '수분 섭취 루틴',
    description: '하루 8잔 건강한 습관',
    category: 'DIET',
    emoji: '💧',
    missions: [
      {
        title: '물 마신 횟수 기록',
        instruction: '오늘 마신 컵 수를 기록해요',
        icon: 'water.png',
        requires_image: false,
        requires_numeric: true,
        requires_note: false,
        verification_type: 'AUTO',
        point: 3,
        daily_limit: 1,
      },
      {
        title: '수분 섭취 한 줄 소감',
        instruction: '컨디션이 어땠나요? 한 줄로 남겨주세요',
        icon: 'water.png',
        requires_image: false,
        requires_numeric: false,
        requires_note: true,
        verification_type: 'AUTO',
        point: 2,
        daily_limit: 1,
      },
    ],
  },

  {
    key: 'mindcare',
    title: '마음챙김 명상',
    description: '마음의 평온을 찾아보세요',
    category: 'MINDCARE',
    emoji: '🧘',
    icon: '/icons/meditation/meditate.png',   // 묶음 카드 3D 아이콘
    missions: [
      {
        title: '3분 명상',
        instruction: '자리를 잡고 앉아 3분간 호흡에 집중해요',
        icon: null,   // 아이콘 없으면 카드가 3D 명상 아이콘을 기본으로 씀
        verify_style: 'meditation',
        meditation_seconds: 180,
        meditation_pattern: { inhale: 4, hold1: 4, exhale: 4, hold2: 4 },  // 박스 호흡
        requires_image: false,
        requires_numeric: false,
        requires_note: false,
        verification_type: 'AUTO',
        point: 10,
        daily_limit: 1,
      },
    ],
  },

  {
    key: 'no_smoking',
    title: '금연 서포트',
    description: '건강한 변화를 응원합니다',
    category: 'NO_SMOKING',
    emoji: '🚭',
    missions: [
      {
        title: '오늘 안 피웠어요',
        instruction: '오늘 흡연하지 않았다면 인증해주세요',
        icon: 'nosmoke.png',
        requires_image: false,
        requires_numeric: false,
        requires_note: true,
        verification_type: 'AUTO',
        point: 10,
        daily_limit: 1,
      },
      {
        title: '금연 의지 한 줄',
        instruction: '오늘 다짐을 한 줄로 남겨주세요',
        icon: 'nosmoke.png',
        requires_image: false,
        requires_numeric: false,
        requires_note: true,
        verification_type: 'AUTO',
        point: 3,
        daily_limit: 1,
      },
    ],
  },

  {
    key: 'exercise_certification',
    title: '운동 인증 챌린지',
    description: '오늘의 운동을 사진과 소감으로 인증해요',
    category: 'WALKING',
    emoji: '💪',
    missions: [
      {
        title: '운동 사진',
        instruction: '오늘 운동한 순간을 사진으로 인증해요',
        icon: 'exercise.png',
        requires_image: true,
        requires_numeric: false,
        requires_note: false,
        verification_type: 'AUTO',
        point: 7,
        daily_limit: 1,
      },
      {
        title: '운동 후 한 줄 소감',
        instruction: '오늘 운동 어땠나요? 한 줄로 남겨주세요',
        icon: 'exercise.png',
        requires_image: false,
        requires_numeric: false,
        requires_note: true,
        verification_type: 'AUTO',
        point: 3,
        daily_limit: 1,
      },
    ],
  },

  {
    key: 'walking',
    title: '걷기 챌린지',
    description: '매일 꾸준한 걷기 습관',
    category: 'RUNNING',
    emoji: '🚶',
    missions: [
      {
        title: '걸음 수 기록',
        instruction: '오늘 걸은 걸음 수를 기록해요',
        icon: 'walk.png',
        requires_image: false,
        requires_numeric: true,
        requires_note: false,
        verification_type: 'AUTO',
        point: 5,
        daily_limit: 1,
      },
      {
        title: '산책 사진',
        instruction: '오늘 산책한 풍경을 사진으로 인증해요',
        icon: 'walk.png',
        requires_image: true,
        requires_numeric: false,
        requires_note: false,
        verification_type: 'AUTO',
        point: 5,
        daily_limit: 1,
      },
      {
        title: '걷기 후 컨디션',
        instruction: '걷기 후 느낌을 한 줄로 남겨주세요',
        icon: 'walk.png',
        requires_image: false,
        requires_numeric: false,
        requires_note: true,
        verification_type: 'AUTO',
        point: 3,
        daily_limit: 1,
      },
    ],
  },

  {
    key: 'empathy_share',
    title: '공감·나눔 루틴',
    description: '주변과 따뜻함을 나눠요',
    category: 'EMPATHY',
    emoji: '🤝',
    missions: [
      {
        title: '고마움 전하기',
        instruction: '오늘 누군가에게 전한 고마움을 한 줄로 남겨주세요',
        icon: 'diary.png',
        requires_image: false,
        requires_numeric: false,
        requires_note: true,
        verification_type: 'AUTO',
        point: 5,
        daily_limit: 1,
      },
      {
        title: '친절 인증 사진',
        instruction: '오늘 베푼 또는 받은 친절을 사진으로 남겨요',
        icon: 'photo.png',
        requires_image: true,
        requires_numeric: false,
        requires_note: false,
        verification_type: 'AUTO',
        point: 7,
        daily_limit: 1,
      },
    ],
  },

  // ─── Day 65 신규 추가 (라이브러리 확장) ──────────────────

  // WALKING 추가 ────────────────
  {
    key: 'running_routine',
    title: '러닝 루틴',
    description: '꾸준한 달리기로 체력 키우기',
    category: 'RUNNING',
    emoji: '🏃',
    missions: [
      {
        title: '러닝 인증',
        instruction: '러닝 후 거리·시간·칼로리를 기록하고, 사진과 한 줄 소감을 남겨요',
        icon: 'running.png',
        requires_image: true,
        requires_numeric: true,
        requires_note: true,
        // 입력별 점수 (사진 5 + 기록 7 + 소감 3 = 15)
        image_point: 5,
        numeric_point: 7,
        note_point: 3,
        // 사진·소감은 선택(가진 것만), 기록은 필수
        image_required: false,
        numeric_required: true,
        note_required: false,
        // 다중 지표 (거리/시간/칼로리) + 개요 통계 표시
        metrics: [
          { key: 'distance', label: '거리', unit: 'km', max: 100, icon: '👟' },
          // HHMMSS(시분초 6자리)로 입력받아 분으로 저장, 통계는 H:MM 으로 표시
          { key: 'time', label: '운동 시간', unit: '분', max: 1440, icon: '🕐', inputFormat: 'hms', sumFormat: 'hm' },
          { key: 'calories', label: '칼로리', unit: 'kcal', max: 5000, icon: '🔥' },
        ],
        metric_aggregate: true,
        verification_type: 'MANUAL',  // 기록 지표(거리/시간/칼로리)는 운영자 심사 후 요약에 반영
        point: 15,
        daily_limit: 1,
      },
    ],
  },

  {
    key: 'stretching_routine',
    title: '스트레칭·유연성',
    description: '굳은 몸을 부드럽게 풀어줘요',
    category: 'WALKING',
    emoji: '🧘‍♀️',
    missions: [
      {
        title: '스트레칭 시간 기록',
        instruction: '오늘 스트레칭한 시간(분)을 기록해요',
        icon: 'stretching.png',
        requires_image: false,
        requires_numeric: true,
        requires_note: false,
        verification_type: 'AUTO',
        point: 5,
        daily_limit: 1,
      },
      {
        title: '스트레칭 인증',
        instruction: '스트레칭 후 컨디션을 한 줄로 남겨요',
        icon: 'stretching.png',
        requires_image: false,
        requires_numeric: false,
        requires_note: true,
        verification_type: 'AUTO',
        point: 3,
        daily_limit: 1,
      },
    ],
  },

  // DIET 추가 ────────────────
  {
    key: 'protein_focus',
    title: '단백질 챙기기',
    description: '근육과 회복을 위한 단백질 습관',
    category: 'DIET',
    emoji: '🍗',
    missions: [
      {
        title: '단백질 식사 사진',
        instruction: '오늘 먹은 단백질 식품을 사진으로 인증해요 (계란/닭가슴살/두부 등)',
        icon: 'diet.png',
        requires_image: true,
        requires_numeric: false,
        requires_note: false,
        verification_type: 'AUTO',
        point: 5,
        daily_limit: 2,
      },
      {
        title: '단백질 섭취량 기록',
        instruction: '오늘 섭취한 단백질량(g)을 기록해요',
        icon: 'diet.png',
        requires_image: false,
        requires_numeric: true,
        requires_note: false,
        verification_type: 'AUTO',
        point: 3,
        daily_limit: 1,
      },
    ],
  },

  {
    key: 'vegetable_challenge',
    title: '채소 챌린지',
    description: '하루 한 끼 이상 채소 챙기기',
    category: 'DIET',
    emoji: '🥦',
    missions: [
      {
        title: '채소 식단 사진',
        instruction: '오늘 채소가 들어간 식사를 사진으로 인증해요',
        icon: 'diet.png',
        requires_image: true,
        requires_numeric: false,
        requires_note: false,
        verification_type: 'AUTO',
        point: 5,
        daily_limit: 2,
      },
      {
        title: '채소 한 줄 소감',
        instruction: '오늘 어떤 채소를 먹었나요?',
        icon: 'diet.png',
        requires_image: false,
        requires_numeric: false,
        requires_note: true,
        verification_type: 'AUTO',
        point: 2,
        daily_limit: 1,
      },
    ],
  },

  // SLEEP 추가 ────────────────
  {
    key: 'bedtime_routine',
    title: '잠자기 전 루틴',
    description: '편안한 숙면을 위한 준비',
    category: 'SLEEP',
    emoji: '🛏️',
    missions: [
      {
        title: '취침 전 스트레칭/명상',
        instruction: '오늘 잠자기 전 5분 이상 스트레칭 또는 명상을 했다면 인증',
        icon: 'sleep.png',
        requires_image: false,
        requires_numeric: false,
        requires_note: true,
        verification_type: 'AUTO',
        point: 5,
        daily_limit: 1,
      },
      {
        title: '취침 시각 기록',
        instruction: '오늘 잠든 시각을 기록해요 (예: 23.5 = 23시 30분)',
        icon: 'sleep.png',
        requires_image: false,
        requires_numeric: true,
        requires_note: false,
        verification_type: 'AUTO',
        point: 3,
        daily_limit: 1,
      },
    ],
  },

  // MINDCARE 추가 ────────────────
  {
    key: 'gratitude_journal',
    title: '감사 일기',
    description: '매일 감사를 기록하는 습관',
    category: 'MINDCARE',
    emoji: '📓',
    missions: [
      {
        title: '오늘 감사한 일 3가지',
        instruction: '오늘 감사했던 일 3가지를 한 줄로 정리해서 남겨주세요',
        icon: 'diary.png',
        requires_image: false,
        requires_numeric: false,
        requires_note: true,
        verification_type: 'AUTO',
        point: 5,
        daily_limit: 1,
      },
    ],
  },

  {
    key: 'stress_relief',
    title: '스트레스 해소',
    description: '나만의 방법으로 스트레스 풀기',
    category: 'MINDCARE',
    emoji: '😌',
    missions: [
      {
        title: '스트레스 해소 활동',
        instruction: '오늘 한 스트레스 해소 활동을 한 줄로 남겨주세요 (음악/산책/취미 등)',
        icon: 'meditation.png',
        requires_image: false,
        requires_numeric: false,
        requires_note: true,
        verification_type: 'AUTO',
        point: 5,
        daily_limit: 1,
      },
      {
        title: '심호흡 시간',
        instruction: '오늘 심호흡한 시간(분)을 기록해요',
        icon: 'meditation.png',
        requires_image: false,
        requires_numeric: true,
        requires_note: false,
        verification_type: 'AUTO',
        point: 3,
        daily_limit: 1,
      },
    ],
  },

  // EMPATHY 추가 ────────────────
  {
    key: 'family_connection',
    title: '가족과 연결',
    description: '하루 한 번 가족에게 마음 전하기',
    category: 'EMPATHY',
    emoji: '👨‍👩‍👧',
    missions: [
      {
        title: '가족 안부 한 줄',
        instruction: '오늘 가족에게 보낸 메시지나 안부를 한 줄로 남겨주세요',
        icon: 'diary.png',
        requires_image: false,
        requires_numeric: false,
        requires_note: true,
        verification_type: 'AUTO',
        point: 7,
        daily_limit: 1,
      },
    ],
  },

  {
    key: 'kind_action',
    title: '하루 한 친절',
    description: '주변에 작은 친절 베풀기',
    category: 'EMPATHY',
    emoji: '💝',
    missions: [
      {
        title: '오늘의 친절 한 줄',
        instruction: '오늘 베푼 또는 받은 친절 한 가지를 한 줄로 남겨주세요',
        icon: 'diary.png',
        requires_image: false,
        requires_numeric: false,
        requires_note: true,
        verification_type: 'AUTO',
        point: 5,
        daily_limit: 1,
      },
    ],
  },

  // ETC 신규 카테고리 ────────────────
  {
    key: 'morning_routine',
    title: '아침 루틴',
    description: '활기찬 하루의 시작',
    category: 'ETC',
    emoji: '🌅',
    missions: [
      {
        title: '아침 기상 시각',
        instruction: '오늘 일어난 시각을 기록해요 (예: 6.5 = 6시 30분)',
        icon: 'diary.png',
        requires_image: false,
        requires_numeric: true,
        requires_note: false,
        verification_type: 'AUTO',
        point: 3,
        daily_limit: 1,
      },
      {
        title: '아침 활동 한 줄',
        instruction: '오늘 아침에 한 첫 활동을 한 줄로 남겨주세요',
        icon: 'diary.png',
        requires_image: false,
        requires_numeric: false,
        requires_note: true,
        verification_type: 'AUTO',
        point: 5,
        daily_limit: 1,
      },
    ],
  },

  {
    key: 'reading_habit',
    title: '독서 습관',
    description: '하루 15분 책 읽기',
    category: 'ETC',
    emoji: '📚',
    missions: [
      {
        title: '독서 시간 기록',
        instruction: '오늘 책 읽은 시간(분)을 기록해요',
        icon: 'reading.png',
        requires_image: false,
        requires_numeric: true,
        requires_note: false,
        verification_type: 'AUTO',
        point: 5,
        daily_limit: 1,
      },
      {
        title: '독서 한 줄 소감',
        instruction: '오늘 읽은 책에서 인상 깊은 한 문장을 남겨주세요',
        icon: 'reading.png',
        requires_image: false,
        requires_numeric: false,
        requires_note: true,
        verification_type: 'AUTO',
        point: 5,
        daily_limit: 1,
      },
      {
        title: '독서 인증 사진',
        instruction: '오늘 읽은 책 페이지를 사진으로 인증해요',
        icon: 'reading.png',
        requires_image: true,
        requires_numeric: false,
        requires_note: false,
        verification_type: 'AUTO',
        point: 3,
        daily_limit: 1,
      },
    ],
  },

  {
    key: 'hobby_time',
    title: '나만의 취미 시간',
    description: '내가 좋아하는 일에 시간 쓰기',
    category: 'ETC',
    emoji: '🎨',
    missions: [
      {
        title: '취미 활동 사진',
        instruction: '오늘 즐긴 취미를 사진으로 남겨요',
        icon: 'photo.png',
        requires_image: true,
        requires_numeric: false,
        requires_note: false,
        verification_type: 'AUTO',
        point: 5,
        daily_limit: 1,
      },
      {
        title: '취미 시간',
        instruction: '오늘 취미에 쓴 시간(분)을 기록해요',
        icon: 'diary.png',
        requires_image: false,
        requires_numeric: true,
        requires_note: false,
        verification_type: 'AUTO',
        point: 3,
        daily_limit: 1,
      },
    ],
  },
]
