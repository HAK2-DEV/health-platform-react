...
const SLIDES = [
  ...
  { emoji: '📊', title: '한눈에 관리해요', body: '참여자 통계·가입 승인·초대까지. 언제든 「운영자 가이드」에서 다시 볼 수 있어요.' },
]
        ...
        <div className="p-4 pt-0">
          {last ? (
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => { onClose(); navigate('/operator-guide') }}
                className="flex-1 py-3 rounded-card-lg border border-emerald-200 text-emerald-700 font-semibold hover:bg-emerald-50 transition"
              >
                운영자 가이드 보기
              </button>
              <button
                type="button"
                onClick={onClose}
                className="flex-1 py-3 rounded-card-lg bg-gradient-to-r from-emerald-400 to-teal-500 hover:from-emerald-500 hover:to-teal-600 text-white font-semibold transition"
              >
                시작하기
              </button>
            </div>
