...
function Overlay({ title, children, onClose, wide }) {
  ...
  useBackButtonClose(true, onClose)  // 하드웨어 뒤로가기 = 닫기(스택 최상단)
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/45" style={overlayStyle} onClick={onClose}>
      <div className={`w-full ${wide ? 'max-w-md' : 'max-w-xs'} max-h-[88vh] overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl`} style={cardStyle} onClick={e => e.stopPropagation()}>
