...
const TIPS = [
  { emoji: '🎯', title: '작은 목표부터', body: '처음부터 무리하지 말고 하루 10분·1,000보처럼 작게 시작해요. 꾸준함이 강도보다 중요해요.' },
  { emoji: '🔔', title: '알림을 켜두세요', body: '인증 리마인드 알림을 켜두면 깜빡임이 줄어 연속 기록이 한결 쉬워져요.' },
  { emoji: '🤝', title: '혼자보다 함께', body: '커뮤니티 피드에서 서로의 인증에 응원을 남기면 동기부여가 오래가요.' },
  { emoji: '📅', title: '내 리듬에 맞게', body: '기간·강도·카테고리가 내 생활 리듬에 맞는 프로그램을 고르면 끝까지 가기 쉬워요.' },
  { emoji: '🌱', title: '빠진 날도 괜찮아요', body: '하루 놓쳐도 다음 날 다시 시작하면 돼요. 완벽보다 회복이 중요해요.' },
]
            ...
            <div className="space-y-2.5 max-h-[58vh] overflow-y-auto -mx-1 px-1">
              {TIPS.map((t, i) => (
                <div key={i} className="flex gap-3 p-3 bg-gray-50 rounded-2xl">
                  <span className="text-2xl flex-shrink-0 leading-none mt-0.5">{t.emoji}</span>
