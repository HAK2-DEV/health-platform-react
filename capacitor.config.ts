...
const config: CapacitorConfig = {
  ...
  plugins: {
    // ⚠️ @capacitor/keyboard 는 제거됨(2026-09-14) — 구형 안드(노트9)에서 키보드 이벤트를 아예 안 쏘는 것을
    //   실측해 JS 는 이 플러그인에 의존하지 않고 lib/nativeKeyboard 의 «포커스 기반 스크롤»로 처리한다.
    //   softInputMode 는 MainActivity 가 OS 버전별로 직접 지정하므로 플러그인이 필요 없다.
    SplashScreen: {
